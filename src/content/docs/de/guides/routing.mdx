...
Astro verwendet **dateibasiertes Routing**, um deine Build-URLs basierend auf dem Dateilayout deines Projektverzeichnisses `src/pages/` zu generieren.

## Navigieren zwischen Seiten

Astro verwendet standardmäßige HTML [`<a>`-Elemente](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a), um zwischen Routen zu navigieren. Es gibt keine Framework-spezifische `<Link>`-Komponente.

```astro title="src/pages/index.astro"
<p>Lies mehr <a href="/about/">über</a> Astro!</p>

<!-- Mit `Basis: "/docs"` konfiguriert -->
<p>Erfahre mehr in unserem <a href="/docs/reference/">Referenz</a>-Bereich!</p> 
```

## Statische Routen

`.astro` [Seitenkomponenten](/de/basics/astro-pages/) sowie Markdown- und MDX-Dateien (md, mdx) im Verzeichnis `src/pages/` werden **automatisch zu Seiten auf deiner Website**. Die Route jeder Seite entspricht ihrem Pfad und Dateinamen im Verzeichnis `src/pages/`.
